...
if ($.isNode()) {
   if (process.env.KXHYHEADER && process.env.KXHYHEADER.indexOf('\n') > -1) {
   kxhyheaderVal = process.env.KXHYHEADER.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   kxhyheaderVal = process.env.KXHYHEADER.split()
  };
  Object.keys(kxhyheaderVal).forEach((item) => {
        if (kxhyheaderVal[item]) {
          kxhyheaderArr.push(kxhyheaderVal[item])
        }
    });
  if (process.env.KXHYADHEADER && process.env.KXHYADHEADER.indexOf('\n') > -1) {
   kxhyadheaderVal = process.env.KXHYADHEADER.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   kxhyadheaderVal = process.env.KXHYADHEADER.split()
  };
  Object.keys(kxhyadheaderVal).forEach((item) => {
        if (kxhyadheaderVal[item]) {
          adheaderArr.push(kxhyadheaderVal[item])
        }
    });

   if (process.env.KXHYADBODY && process.env.KXHYADBODY.indexOf('\n') > -1) {
   kxhyadbodyVal = process.env.KXHYADBODY.split('\n');
   console.log(`您选择的是用换行隔开\n`)
  } else {
   kxhyadbodyVal = process.env.KXHYADBODY.split()
  };
  Object.keys(kxhyadbodyVal).forEach((item) => {
        if (kxhyadbodyVal[item]) {
          adbodyArr.push(kxhyadbodyVal[item])
        }
    });
 }else {
