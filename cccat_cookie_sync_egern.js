...
export default async function (ctx) {
  ...
  if (resp.code == 200) {  
     let rescron = await ctx.http.get(`${$.ql_url}/open/crons?searchValue=autocheckin`, { headers:{"Content-Type": `application/json;charset=UTF-8`, "Authorization": `Bearer ${ql_token}`}});
  ...
  resp =  await ctx.http.put(`${$.ql_url}/open/envs`, {headers:{"Content-Type": `application/json;charset=UTF-8`, "Authorization": `Bearer ${ql_token}`}, body: JSON.stringify(taskIDs)});
  let resrun = await resp.json();	  
  console.log("执行响应：")
  console.log(resrun)
